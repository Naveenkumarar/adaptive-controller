...
class QcarNode(object):
    ...
    def looping(self):
        while not rospy.is_shutdown():
            ...
            # talk to QCar
            #rospy.loginfo(f"Current command = {self.command}")
            self.command_time = rospy.Time.now()
            rospy.loginfo(f"Command sent = {self.command_sent}")
            if not self.command_sent:
                rospy.loginfo(f"Vel comm = {self.command[0]}, steer comm = {self.command[1]} \n")
            ...
            if not (self.error_processing_time < self.callback_time):
                ...
                #self.process_cmd(speed=0.07, steering_angle=0.0)
    ...
    def error_callback(self, data):
        rospy.loginfo("New error received. \n")
        self.command_sent = False
        self.callback_time = rospy.Time.now()
