...
from autoware_msgs.msg import ControlCommand, ControlCommandStamped
...
class MicronoleIONode(object):
    ...
    def __init__(self, publish_odometry=False, publish_imu=True, publish_rate=500.0,
        ...
        self.ctrl_sub = rospy.Subscriber("/ctrl_cmd_joy", ControlCommand, self.ctrl_callback,
                                         queue_size=100)
        self.ctrl_sub = rospy.Subscriber("/ctrl_cmd", ControlCommandStamped, self.ctrl_stamped_callback,
                                         queue_size=100)
        #self.ctrl_stamped_sub = rospy.Subscriber("/ctrl_raw", ControlCommandStamped, self.ctrl_stamped_callback,
        #                                 queue_size=100)
    ...
    def ctrl_stamped_callback(self, data):
        frame_id = data.header.frame_id
        timestamp = data.header.stamp
        linear_velocity = data.cmd.linear_velocity  # currently is pwm
        #linear_velocity = 2.3  # m/s
        # todo: convert desired velocity to pwm mapping
        steering_angle = data.cmd.steering_angle - 0.01 + self.steering_offset
        self.command = np.array([linear_velocity, steering_angle])
