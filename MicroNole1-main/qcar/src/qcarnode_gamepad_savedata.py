...
class QcarNode(object):
    ...
    def looping(self):
        while not rospy.is_shutdown():
            self.current_time = rospy.Time.now()
            # rospy.loginfo(f"Time diff = {(self.current_time - self.last_time)}")
            ...
            # rospy.loginfo(f"Timestep: {self.dt}")
            ...
            # rospy.loginfo(f"Quanser Encoder Speed = {encoderCounts_diff2}, \n Encoder Counts diff = {encoderCounts_diff}")
            ...
            # rospy.loginfo(f"Longitudinal speed = {longitudinal_car_speed}")
            ...
            data = np.array([[self.dt,self.command[0], self.command[1], ax,ay,az, roll_rate, pitch_rate, yaw_rate, current, batteryVoltage, encoderCounts]])
    ...
    def process_cmd(self, sub_cmd):
        ...
        str_cmd = sub_cmd.vector.y - 0. * 0.01  # why is the -0.01 term there? Feedforward?
