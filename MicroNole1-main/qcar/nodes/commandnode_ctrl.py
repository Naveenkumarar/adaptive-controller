...
class CommandNode(object):
    def __init__(self, publish_rate=500.0):
        ...
        self.cmd_pub_ = rospy.Publisher('/ctrl_cmd', ControlCommand, queue_size=100)
    ...
    def process_command(self, pose, new):
        if new:
            ...
            # ctrl_cmd.header.frame_id = "command_input"
            max_speed = 2.3
            quanser_saturation = 0.3
            ctrl_cmd.linear_velocity = float(pose[0]) * max_speed / quanser_saturation  # currently in pwm
            ctrl_cmd.steering_angle = float(pose[1])
            self.cmd_pub_.publish(ctrl_cmd)
...
def main(args):
    ...
    rospy.on_shutdown(controller_instance.shutdown)
