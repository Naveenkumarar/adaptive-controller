...
class racingNode(object):
    ...
    def __init__(self, publish_joy=True, publish_rate= 120.):
        ...
        self.ctrl_pub_np = rospy.Publisher('/racing_cockpit/ctrl_cmd_np', Vector3Stamped, queue_size=10)
    ...
    def publisher_joy(self):
        joy_command_np = Vector3Stamped()
        joy_command_np.header.stamp = rospy.Time.now()
        joy_command_np.header.frame_id = 'remote racing'
        joy_command_np.vector.x = float(self.command[0])
        joy_command_np.vector.y = float(self.command[1])
        joy_command_np.vector.z = float(self.command[2])
        
        self.command = (self.b/2)**0.5 * self.command
        joy_command = Vector3Stamped()
        joy_command.header.stamp = rospy.Time.now()
        joy_command.header.frame_id = 'remote racing passive'
        ...
        time.sleep(0.5)
        self.ctrl_pub_np.publish(joy_command_np)
