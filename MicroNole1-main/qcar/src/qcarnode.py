...
class QcarNode(object):
    ...
    def __init__(self, publish_odometry=True, publish_imu=True, publish_odom_tf = True):
        super().__init__()
        ...
        self.odom_pub = rospy.Publisher('/filter/odometry', Odometry, queue_size=10)
        ...
        self.desired_longitudinal_speed = 0.5     # desired longitudinal speed
    ...
    def looping(self):
        while not rospy.is_shutdown():
            ...
            sensor_data = np.array([self.filter_time, float(self.command[0]), float(self.command[1]), encoderCounts, yaw_rate +1 , ax, ay])     # [time, PWM, steering, encoder counts, raw rate, ax, ay]
            ...
            self.filter_data = np.concatenate((self.filter_data,np.array([[self.filter_time, wz_hat, wm_hat, vx_hat, vy_hat, self.x, self.y, self.Phi]]) ),axis=0)

            if self.publish_odometry:
                self.publish_filter_odometry(vx_hat, vy_hat, wz_hat)
    ...
    def publish_filter_odometry(self, vx, vy, yaw_rate):
        base_frame_id = 'base'
        odom_frame_id = 'odom'
        # since all odometry is 6DOF we'll need a quaternion created from yaw
        self.odom_quat = tf.transformations.quaternion_from_euler(0, 0, self.Phi)
    ...
    def lane_following_control(self,e,vx_hat,wm_hat,wz_hat):
        ...
        yaw_rate_desired = self.kl1 * e             +      2*np.sign(e)*self.d0  # the term gain can be 2 or other value, just need to perfrom like damping
    ...
    def kinematic_constraint(self,longitudinal_speed,yaw_rate_desired):
        ...
        if abs(self.steering) <= 0.01:
            longitudinal_speed_update = longitudinal_speed
            yaw_rate_desired_update = yaw_rate_desired
        else:
            q1, q2 = 1.0, 1.0
            Q = np.array([[q1, 0.],[0., q2]])
            a = np.array([[self.L/math.tan(self.steering)],[1.0]])
            
            x_star = np.array([[longitudinal_speed],[yaw_rate_desired]])
            
            opt_yaw_rate = np.dot(np.dot(x_star.T, Q),a)/np.dot(np.dot(a.T, Q),a)
            bound = longitudinal_speed * math.tan(self.steering)/self.L
            
            if opt_yaw_rate <= bound:
                yaw_rate_desired_update = opt_yaw_rate
            else:
                yaw_rate_desired_update = bound

            longitudinal_speed_update = self.L*yaw_rate_desired_update / math.tan(self.steering)
        '''
        '''
        x_star = np.array([[longitudinal_speed],[yaw_rate_desired]])
        ...
        q1, q2 = 1.0, 1.0
        Q = np.array([[q1, 0.],[0., q2]])
        a = np.array([[1.0], [math.tan(self.sat_steering)/self.L]])
        
        opt_speed = np.dot(np.dot(x_star.T, Q),a)/np.dot(np.dot(a.T, Q),a)
        if abs(opt_speed) < abs(longitudinal_speed): 
            longitudinal_speed_update = opt_speed
        else:
            longitudinal_speed_update = longitudinal_speed
        
        yaw_rate_desired_update = (math.tan(self.steering)/self.L) * longitudinal_speed_update
        '''
    ...
    def detection_time_sub(self,meg):
        self.detection_time_stamp = abs(meg.data)
        if meg.data < 0:
            self.clockwise_flag = 0
